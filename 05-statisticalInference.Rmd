...
```{r}
summarySE(captopril,measurevar = "deltaSBP")
```
...
res<-rbind(
femSamp10 %>% log%>%as.data.frame %>% gather(sample,logChol) %>% summarySE(measurevar="logChol",groupvars="sample"),
femSamp50 %>% log %>%  as.data.frame %>% gather(sample,logChol) %>% summarySE(measurevar="logChol",groupvars="sample"),
femSamp100 %>% log %>% as.data.frame %>% gather(sample,logChol) %>% summarySE(measurevar="logChol",groupvars="sample")
)

res %>% 
ggplot(aes(x=N%>%as.factor,y=logChol)) +
geom_boxplot() +
...
1. Redo simulation study for BMI, what do you observe? 

```{r}
femBmi <- NHANES %>% filter(Gender=="female"&!is.na(BMI)) %>% select(BMI)
femBmi %>% ggplot(aes(x=BMI))+
   geom_histogram(aes(y=..density.., fill=..count..)) +
   xlab("BMI") +
   ggtitle("All females in study")

femBmi %>% ggplot(aes(sample=BMI)) +
 stat_qq() +
 stat_qq_line() +
 ggtitle("All females in Nhanes study")

femBmiSamp5<-sapply(1:1000,function(j,x,size) sample(x,size),size=5,x=femBmi$BMI) 

femBmiSamp5 %>% colMeans %>% as.data.frame %>%  ggplot(aes(sample=.)) +
stat_qq() +
stat_qq_line() +
ggtitle("Means on 5 females")

femBmiSamp10<-sapply(1:1000,function(j,x,size) sample(x,size),size=10,x=femBmi$BMI) 

femBmiSamp10 %>% colMeans %>% as.data.frame %>%  ggplot(aes(sample=.)) +
stat_qq() +
stat_qq_line() +
ggtitle("Means on 10 females")

femBmiSamp50<-sapply(1:1000,function(j,x,size) sample(x,size),size=50,x=femBmi$BMI) 

femBmiSamp50 %>% colMeans %>% as.data.frame %>%  ggplot(aes(sample=.)) +
stat_qq() +
stat_qq_line() +
ggtitle("Means on 50 females")
```

2. What happens when you log transform BMI, assess for sample sizes 5 and 10? 
...
```{r}
res$ll<-res$logChol-1.96*res$se
res$ul<-res$logChol+1.96*res$se
...
```{r}
res %>% filter(N==10) %>% slice(1:20) %>% ggplot(aes(x=sample,y=logChol,color=inside)) + 
  geom_point() +
  geom_errorbar(aes(ymin=logChol-1.96*se,ymax=logChol+1.96*se))+
  geom_hline(yintercept = fem$DirectChol%>% log %>% mean) + 
...
```{r}
res %>% filter(N==50) %>% slice(1:20) %>% ggplot(aes(x=sample,y=logChol,color=inside)) + 
  geom_point() +
  geom_errorbar(aes(ymin=logChol-1.96*se,ymax=logChol+1.96*se))+
  geom_hline(yintercept = fem$DirectChol%>% log %>% mean) +
...
res %>% filter(N==100) %>% slice(1:20) %>% ggplot(aes(x=sample,y=logChol,color=inside)) + 
  geom_point() +
  geom_errorbar(aes(ymin=logChol-1.96*se,ymax=logChol+1.96*se))+
  geom_hline(yintercept = fem$DirectChol%>% log %>% mean) +
...
res$ll<-res$logChol-qt(0.975,df=res$N-1)*res$se
res$ul<-res$logChol+qt(0.975,df=res$N-1)*res$se
mu<-fem$DirectChol%>%log%>%mean
res$inside<-res$ll<=mu & mu<=res$ul
res$N <- as.factor(res$N)
res %>% group_by(N) %>% summarize(coverage=mean(inside)) %>% spread(N,coverage) 
...
res %>% filter(N==10) %>% slice(1:20) %>% ggplot(aes(x=sample,y=logChol,color=inside)) + 
  geom_point() +
  geom_errorbar(aes(ymin=logChol-qt(0.975,df=9)*se,ymax=logChol+qt(0.975,df=9)*se))+
  geom_hline(yintercept = fem$DirectChol%>% log %>% mean) + 
  ggtitle("20 CI for N=10") + 
  ylim(range(fem$DirectChol %>% log))
```
