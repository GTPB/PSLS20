...
- The CMET-groep at Ghent University does research to the use of microbial transplants of the armpit to save people with smelly armpits. 
...
- Experiment: 
    ...
    - transplantie (antibiotica, gevolgd door microbi??le transplantatie).
...
```{r}
ap<-read_csv("https://raw.githubusercontent.com/statOmics/psls20/master/data/armpit.csv?token=AF2MWG5H3QKFBWLFS4SDQSK6D4T2M")
ap
```
...
2. A research assistant selects 20 people with smelly armpits from the Belgian population. 
...
- For the statistical analysis to be valid, it is required that the subjects are selected completely at random.
...
- Selecting completely at random from a population implies:
    ...
    - the selection of a subject in the sample should be independent from the selection of the other pears in the sample.
...
National Health NHanes study
  ...
  - Complete the health examination component of the survey, which is conducted in a mobile examination centre (MEC).
  - We will use this larger study to select random subjects from the American population. 
...
```{r}
NHANES%>%filter(NHANES$Age>25)%>%ggplot(aes(x=DirectChol))+geom_histogram() +facet_grid(Gender~.) + xlab("Direct Cholestorol (mg/dl)")
```
...
```{r}
NHANES%>%filter(NHANES$Age>25)%>%ggplot(aes(x=DirectChol%>%log))+geom_histogram() +facet_grid(Gender~.) + xlab("Direct Cholestorol (log)")
```

We see that upon log transformation they are more or less bell shaped. 

```{r}
nhanesSub<-NHANES%>%filter(Age>25&!is.na(DirectChol)) %>% select(c("Gender","DirectChol")) %>% mutate(cholLog=log(DirectChol))
Rmisc::summarySE(measurevar = "cholLog","Gender",data=nhanesSub)
```
...
samp<-rbind(fem,mal)
samp%>%ggplot(aes(x=DirectChol%>%log))+geom_histogram(binwidth = .1) +facet_grid(Gender~.) + xlab("Direct Cholestorol (log)")
samp%>% ggplot(aes(x=Gender,y=cholLog)) + geom_boxplot(outlier.shape=NA) + geom_point(position="jitter") 
Rmisc::summarySE(measurevar = "cholLog","Gender",data=samp)
...
mal<-nhanesSub%>%filter(Gender=="male")%>%sample_n(size=10)
samp2<-rbind(fem,mal)
samp2%>%ggplot(aes(x=DirectChol%>%log))+geom_histogram(binwidth = .1) +facet_grid(Gender~.) + xlab("Direct Cholestorol (log)")
samp2%>% ggplot(aes(x=Gender,y=cholLog)) + geom_boxplot(outlier.shape=NA) + geom_point(position="jitter") 
Rmisc::summarySE(measurevar = "cholLog","Gender",data=samp2)
...
## Summary
- Because we sampled other subject, we obtain diffent cholestorol levels. 
- However, not only the cholestorol levels differ from experiment to experiment but also the summary statistics: means, standard deviations and standard errors. 
...
- So also our conclusions are random and might change from sample to sample. 
...
Run the code above and modify the sample size to 50.
What happens?

Note, that we can write more efficient code to do the simulation above.

# Salk study

- In 1916 first large polio epidemy in the US
- John Salk developed a vaccine with promising results in the lab
- In 1954 the National Foundation
for Infantile Paralysis (NFIP) has setup a large study to assess effectiveness of vaccine.
...
##NFIP study
###Design
- Large simultaneous study with cases, vaccinated, and controls,  non-vaccinated children
...
- controls: 750000 children from  second and third grade
...
- We observe a lower incidence for children for who no consent was given than in the control group
- consent for vaccination was associated with the socio-economic status
...
A new study was conducted: Randomized double blind study
  ...
  - Children are assigned at random to control or case treatment after consent was given by the parents
  - Control: vaccination with placebo
  - Treatment: vaccination with vaccine
  - double blinding: 
    - parents did not know if child was vaccinated or received the placebo
    - care-giver/researchers did not know if child was vaccinated  or recieved placebo
...
- Incidence for children with no consent remains similar, `r nfip$incidencePM[3]` and `r salk$incidencePM[3]` per million in the NFIP and Salk study, respectively. 
...
- *Data-exploration and analysis* can to refine the theory to generate new hypotheses. Scientific cycle.
...
- We have seen that 
    ...
    - be aware of confounding
    - So good experimental design is crucial!
...
- Statistics is the science on 
    ...
    3. learning from data and to generalize what we observe in the sample towards the population (statistical modelling and statistical inference) while quantifying, controlling and reporting variability and uncertainty. 
