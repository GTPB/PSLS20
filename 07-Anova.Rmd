...
Researchers study the effect of arachidonic acid on prostacyclin level in blood plasma. They use 3 different concentrations of arachidonic acid: low, medium and high dose and measure the prostacycline levels in blood plasma using an elisa fluorescence measurement.
...
Data in three groups is approximately Normally distributed with equal variance:
...
Each hypothesis can be testedd via two-sample t-tests $\rightarrow$ multiple testing problem + loss of power.
...
- $Y_i$ de outcome for observatie $i$ ($i=1,\ldots, n$)
\vspace{7pt}
...
0 & \text{if observation $i$ belongs to other dose group} \end{array}\right. .$$
\vspace{7pt}
 - Low dose group (L) with $x_{i1}=x_{i2}=0$ is  *referencd group*
...
\vspace{-20pt}
 ...
 Original null hypothese $H_0:\mu_1=\mu_2=\mu_3$ can be formulated as
 $$H_0: \beta_1=\beta_2=0.$$

Model allows to use all methods we derived for linear regression models
...
### Decomposition of Total Sum of Squares
 ...
 - The convention in the Anova setting is to denote the sum of squares as SST, de **\alert{Sum of Squares of the Treatment (treatment)}** or as SSBetween.
- The sum of squares of the regression indeed reflects the variability between the groups. 
- The corresponding mean som of squares becomes  $\text{MST}=\text{SST}/2$.
...
- The methode does not allow to identify which means are different.

A first naive method is to split $H_0$  in partial hypotheses
$$H_{0jk}: \mu_j=\mu_k \text{ versus } H_{1jk}: \mu_j \neq \mu_k$$

- falsify partiale null hypotheses with two-sample $t$-testen
...
- The $t$-testen are thus best based on
...
5. We rapport th erelative frequency of rejection of the global null hypothesis, i.e. the probability on a type I error  $H_0: \mu_1=\mu_2=\mu_3$.
...
- The simulation study illustrates the  **multiplicity** problem
  ...
  - Her the final decision is based on $m=g\times(g-1)/2$ $p$-values.

- We first discuss on the extension of the concept of type I errors and then introduce some solutions

## Family-wise error rate

- When $m>1$ tests are used to from one decision it is necessary to correct for the risk on false positive results (type I errors).
- Most procedures for multiple testing assume that *alle $m$ null hypotheses are true*.
...
\begin{eqnarray*}
\alpha_F&=&\text{P}[\text{minstens 1 Type I fout}]\\
&=&1-(1-\alpha)^m \leq m\alpha
\end{eqnarray*}

- If we assess 5 test on the 5% significance level then  the FWER $\approx 25\%$. {10pt}
- By conducting then at the 1% significance level the FWER $\approx 5\%$.

- The Bonferroni correction controls the FWER on $\alpha_F$ by setting $$\alpha=\alpha_E/m$$ for each of the $m$ pairwise comparisons
...
An alternative approach is to report
  ...
  2. and $(1-\alpha_E/m)100\%$ confidence intervals.
...
- The conclusions remain similar, except that the FWER is now controled at $\alpha_E=5\%$ and the $\tilde{p}$-values are larger with a factor 3. 
...
- The function below is just here for completeness, but we will generally use the default method for multiple testing in multcomp.
...
### Evaluate Bonferroni methode via simulation
...
- is the default in the multcomp package:
...
1. Anova before posthoc tests: F-test has a higher power than  pairswise t-test

    - F-test uses all data
    - for F-test we do not need to correct for multiple testing: one test for the general omnibus hypothesis
