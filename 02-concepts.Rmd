---
title: "Introduction: why do we need statistics"
output:
    html_document:
      code_download: true
      theme: cosmo
...
## Experimental Design (1)
   - Researcher determines the **populatie** to which they want to generalize their conclusions to.
   - Financial en logistic limitations $\rightarrow$ **representative sample** from population
   
## Data analysis (2 & 3)
   - **Data-Exploration en Descriptive Statistics (2)**: explore, visualize, summarize, gain insight, check assumptions
   - **Statistical Inference (3)**: Generalize what we observe in the sample towardt the population so that we can draw general conclusions on the biological process we study on population level based on the sample in our study. We need statistical models to analyze the data and to quantify and report on variability and uncertainty.  
   
# Example

- National Health and Nutrition Examination Survey (NHANES)
- American demografic studie
- Large number of physical, demographic, nutritional, life style and health characteristics are collected in this study. 
...
- It varies from subject tot subject in the population and thus in the study.


## *Types* of variabeles

1. *Qualitative variabelen* a limited number of outcome catergories, non-numeric. 
	-  *nominal variabeles*: no natural ordening, e.g. gendere, race, blood group, eye color, ... 
...
2. *Numeric variabeles*:
	...
	- Often dichotomised to make nominal $\rightarrow$ information loss
...
- Population is statistics is a theoretical concept
...
- Populatie has to be explained clearly   
...
- Variabelen are thus *random* because their value changes in the population 

- **Crucial question**: Hou precise are the conclusions on the population based on a group of subjected that are in a sample! 
...
- Typically a sequence of random variables $X_1,\ldots X_n$ will be collected in the study. 

- They are necessary to reason on how the results and conclusions can change from sample to sample. 
...
- An event $A$  is a subset of the sample space: e.g. get an even number when roling a dice: $A=(2,4,6)$. Can also be a  $A=(1)$ one  subset of the sample space. 

- Event space $\mathcal{A}$ is the class of all possible events associated with a given experiment.

- Two events are multiple exclusive if they cannot occur together e.g. event of the odd numbers $A_1=(1,3,5)$ and the event of getting $A_2=(6)$. So $A_1 \bigcap A_2=\emptyset$.

- Probability $P(A)$  is a function $P: A \rightarrow [0,1]$ which satisfies
    
    1. $P(A) \leq 0$ and $P(A) \leq 1$ for each $A \in \mathcal{A}$
    2. $P(\Omega)=1$
...
- If we draw to subjects (j and k) independently from the population then the joint probability on  
...
- Example Gender is a binary variable (0:male, 1:female) and can be using a Bernouilli distribution. 50.2% of the subjects of the American population are female and 49.8% are male. Let $\pi$ be the probability on a female $pi=0.502$. 
    so 
    $$ X\sim \left \{ \begin{array}{lcl}
    P(X=0) &=& 1-\pi
    P(X=1) &=& \pi
    \end{array} \right . $$
    
    ```{r}
    plot(c(0,1),c(0.502,0.498),ylim=c(0,1),type="h",xlab="Gender",ylab=c("Probability"),xlim=c(-.3,1.3))
    ```
    Random variable $X$ follows an Bernouilli distribution $B(\pi)$ with parameter $\pi=0.502$ 
    $$B(\pi)= \pi^X(1-\pi)^(X-1)$$

#### Cumulative distribution function

- The cumulative distribution function is the function F(x) that calculates the probability to observe a random variable X for which holds $X\leq x$: 
...
- Many biological characteristics are approximatively normally distributed (upon transformation) $f(x) = N(\mu,\sigma^2)$
...
$$F(x)=\int \limits_{X\leq x } f(x) dx$$

```{r}
plot(grid,pnorm(grid,100,15),type="l",xlab="IQ",ylab="Probability")
```

So the probability that the IQ of a random subject is below 80 can be obtained by

```{r nhanesNormalMetKans, fig.asp=.8, fig.align='center',echo=FALSE}
grid2<-seq(40,80,.01)
plot(grid,dnorm(grid,mean=100,sd=15),
     xlab="IQ",
     col=2,ylab="Densiteit",type="l",lwd=2,cex.lab=1.5,cex.axis=1.5)
...
The SD for a normal distribution, $\sigma$ has the nice interpretation that approximately 68% of the population has a characteristic X in the interval of \sigma around the mean: 
...
- For normally distributed random variables approximately 95% of the subjects in the population have a value between two standard deviations of the mean
...
- Upon standardizatio the data follow a standard normal distribution with mean $\mu=0$ and variance $\sigma^2=1$:
$$z \sim N(0,1)$$

We can use the qnorm function to calculate the quantile $z_{2.5\%}$ that corresponds to $F(z_{2.5\%})=0.025$. 
...
This is indeed  indicates that about 97.5%-2.5%=95% of a standard normal random variable falls within the interval [-2,2], within 2 times the standard deviation from the mean.

# Sample

- For practical problems we do not know the distribution in the population

- Due to financial and logistic reasons we can almost never study the entire population 

- Populationparameters (e.g. mean IQ, variance of IQ)  can therefore not be obtained without error
...
- The distribution in the population has to be estimated. 

- If we can assume that the studied characteristic follows a particular distribution then we only have to estimate the population parameters based on the sample, which we call estimates. 
...
- Select $n=10000$ subject at random from the American popultion
...
- $x$ is a particular value and no longer an unknown variable
...
- We only have to estimate two parameters: the mean and the variance. 
...
``` 
    ...
    - Note, that this estimate is very crude based for the small sample! We do not have enough observations to have a good approximation of the extreme quantiles. 
    
### Normal approximation

- We can use the function qnorm to calculate quantiles from the normal distribution. 

- We also know that it is rougly in two standard deviations from the mean. 
...
- For the large sample empirical distribution (quantile function) and the normal approximation gives us approximately the same result.
...
- The numeric result obtained by evaluating these formulated results are also called statistics or estimates. 

- e.g. the sample mean $\bar X$

- Researcher want to known the unknown parameters from the population and will estimate them using the statistics observed or calculated based on the sample 
...
- So when we analyse data we have to reason on how they will vary from sample to sample. 

- When the statistics refer to a numeric value realised in a particular sample we will use a small letter: $\bar x$ and $s^2$.s
