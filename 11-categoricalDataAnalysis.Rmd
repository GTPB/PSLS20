...
```{r}
beatles<-read_csv("https://raw.githubusercontent.com/GTPB/PSLS20/master/data/beatles.csv")
head(beatles)
table(beatles$dose,beatles$status)
```
...
```{r}
beatleModel<-glm(status~dose,data=beatles,family=binomial)
summary(beatleModel)
```
...
beatlesTab<-table(beatles) %>% data.frame

data.frame(grid=seq(min(beatles$dose),max(beatles$dose),.1)) %>% 
  ...
  geom_text(aes(x=dose%>%as.character%>%as.double,y=status%>%as.character%>%as.double,label=Freq),beatlesTab%>%filter(status==0)) +
geom_text(aes(x=dose%>%as.character%>%as.double,y=status%>%as.character%>%as.double,label=Freq),beatlesTab%>%filter(status==1))
```
