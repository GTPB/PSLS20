...
- Retrospective study
- Impossible to assess risks and risk-differences on breastcancer because the proportion of cases and controls does not reflect that in the population!
...
- Women with breast cancer have a probability on the Leu/Leu genotype that is 14% higher than than of women without borstkanker.
- It suggests an association, but it does not let us conclude how much higher the risk is on breastcancer for women with the Leu/Leu genotype as compared to the other women. 
- Other statistic?
...
- Cases: $\mbox{odds}_1=\frac{ f/(d+e+f)}{(d+e)/(d+e+f)}=f/(d+e)=89/711=0.125$. Women with breastcancer have a probability that is 8 times higher to have another allele combinantion than to have the double Leu/Leu variant of the BRCA gene.
...
$$H_0: \text{Er is no association between } X \text{ and } Y \text{ vs } H_1: X \text{ and } Y \text{ are associated}$$

- Consider row totals $n_\text{other}=a+c$, $n_\text{leu,leu}=b+d$ and
- columntotals $n_\text{contr}=a+b$ en $n_\text{case}=c+d$.
...
- Under $H_0$ $X$ and $Y$ are independent and one expects the proportie $(b+d)/n$
of the $a+b$ controls to have a Leu/Leu variant, or that $(a+b)(b+d)/n$ of them has a Leu/Leu variant 
...
- A large value of the test statistic give an indication that the null hypothesis is false.
...
- Because $O_{ij}$ are discrete values, the $X^2$ can only take discrete values and the continuous slechts $\chi^2_1$-distribution is only an approximation of the real distribution. 
...
- When the correction is not used (i.e. when the values of `0.5' in the estimator $X^2$ are replaced) we use the *Pearson Chi-kwadraat toets*.

---

In R you can switch the correction on or of by setting the argument \texttt{correct} on TRUE or FALSE, respectively:
...
- $\chi^2$-test can also be used if one of the categorical variables $X$ and $Y$ has more then 2 levels

- Again: the null hypothesis $H_0: X$ and $Y$ are independent (not asociated), against the alternative $H_A: X$ and $Y$
are associated.

- Let the variable in the rows have $r$ possible different outcomes and the one in the columns $c$ possible outcomes, then we obtain an $r \times c$ table.
...
- The statistic is now `r format(chisq.test(brcaTab)$statistic,digits=4)` and follows a $\chi^2$ distribution with `r chisq.test(brcaTab)$parameter` degrees of freedom under $H_0$. The probability to obtain a $\chi^2$-test statistic in a random sample uner $H_0$ that is more extreme than `r format(chisq.test(brcaTab)$statistic,digits=4)`, is`r format(chisq.test(brcaTab)$p.value*100,digits=2)`%.
- On the 5% level of significance we conclude that the variant of the BRCA-gene is not associated with breastcancer. 

---

# Logistic regression

- Framework to model binairy data (e.g cancer vs no cancer): *logistic regression model*.
- Model binair data with continuous and/or dummy variables.

- The models assume that observations for subject $i=1,\ldots,n$ are independent and follow a Bernoulli distribution.
...
- Breastcancer example: is BRCA 1  variant associated with breastcancer.
...
The intercept is the log-odds on cancer in the reference class (Pro/Pro) and the slope terns are  log odds ratios between treatment and reference class:
\begin{eqnarray*}
...
The $\chi^2$-test on the logsitic regression model also indicates that there is no significant association between outcome (cancer status) and factor (genetic variant of the BRCA gene) ($p=$ `r format(anova(brcaLogit,test="Chisq")[2,"Pr(>Chi)"],digits=3)`).
De p-value is almost equivalent to the one of the  $\chi^2$-test in the previous section.

---

- Significant association? Post-hoc tests to evaluate which of the odds ratios are different from 1.
...
- Toxicolocal effect of carbon disulfite (CS$_2$) on beatles.
- Research hypothesis is there an effect of the  CS$_2$ concentration on the mortality of beatles?

##Design##
- 32 independent experiments
- Each time 1 beatle is exposed to one of 8 CS$_2$ concentrations (mg/l).
- The outcole: mortality ($y=1$) or survival ($y=0$).

```{r}
beatles<-read_csv("https://raw.githubusercontent.com/GTPB/PSLS20/master/datadata/beatles.csv")
head(beatles)
table(beatle$dose,beatle$status)
```
...
```{r}
beatleModel<-glm(status~dose,data=beatle,family=binomial)
summary(beatleModel)
```
...
- Note that this is a very large extrapolation: minimum dose in dataset is `r min(beatles$dose)` mg/l.


- Estimated odds ratio for the effect of dose on the mortality probability is $\exp(0.3013)=1.35$.
- So beatles expose to a CS$_2$ concentration that is 1 mg/l large, will on average have an odds ratio on mortality of $1.35$.
...
```{r}
data.frame(grid=seq(min(beatle$dose),max(beatle$dose),.1)) %>% 
  mutate(piHat=predict(beatleModel,
	      newdata=data.frame(dose=doseGrid),
	      type="response")) %>% 
...
```{r,echo=FALSE}
plot(doseGrid, piHat, type="l",ylim=c(0,1), xlab="dose", ylab="Prob(dood)",cex.lab=1.5,cex.axis=1.5)
#because we observe multiple beatles per dose,
#we calculate how manu beatles survived for each dose
#and we add this to the plot
text(as.double(rownames(tabbeatles)),0,labels=tabbeatles[,1],cex=1.5)
text(as.double(rownames(tabbeatles)),1,labels=tabbeatles[,2],cex=1.5)
```
