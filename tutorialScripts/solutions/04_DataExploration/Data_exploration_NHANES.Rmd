...
visualize data. We will work with one speficic dataset;
...
have a lookt at the preliminary_tidyverse.Rmd file!
...
that the BMI colum in the original dataset was computed
...
** Note: we are currently calculating the standard deviation on the **
** mean value, but each value is a mean of three measurements. **
...
summary_NHANES <- NHANES %>%
  ...
  distinct(ID,.keep_all=TRUE) %>% ## removes duplicated IDs, retains 1467 subjects
  filter(Smoke100n == "Non-Smoker", Diabetes == "No", HardDrugs == "No", HealthGen != "Poor", SleepTrouble == "No", BMI_WHO%in%c("18.5_to_24.9","25.0_to_29.9")) %>%
  summarise(log2BPSysAve=BPSysAve%>%log2%>%mean,
            sd=BPSysAve%>%log2%>%sd,
            n=is.na(BPSysAve)%>% `!` %>%sum) %>%
  mutate(se=sd/n)

paste("Mean value (log2):", summary_NHANES$log2BPSysAve)
paste("Geometric mean value (log2):", 2^summary_NHANES$log2BPSysAve)
paste("Standard deviation (log2):", summary_NHANES$sd)
paste0("Reference interval (log2): [", summary_NHANES$log2BPSysAve - 2*summary_NHANES$sd, ";", summary_NHANES$log2BPSysAve + 2*summary_NHANES$sd, "]")
paste0("Reference interval (log2): [", 2^(summary_NHANES$log2BPSysAve - 2*summary_NHANES$sd), ";", 2^(summary_NHANES$log2BPSysAve + 2*summary_NHANES$sd), "]")
```
...
The log2 mean blood pressure value of the healthy subset is `r round(summary_NHANES$log2BPSysAve,2)`, with
...
95% reference interval of [`r round(summary_NHANES$log2BPSysAve - 2*summary_NHANES$sd,2)`;`r round(summary_NHANES$log2BPSysAve + 2*summary_NHANES$sd,2)`] on log scale and [`r round(2^(summary_NHANES$log2BPSysAve - 2*summary_NHANES$sd),2)`,`r round(2^(summary_NHANES$log2BPSysAve + 2*summary_NHANES$sd),2)`] mmHg on the original scale. 
