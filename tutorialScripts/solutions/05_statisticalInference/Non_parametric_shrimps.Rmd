---
title: "Tutorial 3.1: Non-parametric analysis on the shrimps dataset"   
output:
    html_document:
      code_download: true    
      theme: cosmo
...
```{r}
shrimps <- read.table("shrimps.txt")
```
...
table(shrimps$group) ## 18 samples in each group
```
...
par(mfrow=c(1,2)) ## to get both QQ plot next to each other
car::qqPlot(PCB_1, main="QQ-plot of PCB_1")
car::qqPlot(PCB_2, main="QQ-plot of PCB_2")
```

We clearly see that we have strong deviations from
normality. Many datapoints do not falls nicel within
...
```{r}
var.test(PCB_1,PCB_2)
```

The null hypothesis that the true ratio of variances 
equals 1 (i.e. that the variances are equal) is rejected.

As such, we are not allowed to continue performing the
t-test. As an alternative, we may perform a non-parametric
