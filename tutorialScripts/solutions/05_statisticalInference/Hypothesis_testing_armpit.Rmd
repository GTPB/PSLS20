...
equal to 0. We have found a p-value of `r round(output$p.value,5)`, which
is much smaller than 0.05. So, the probability of at most `r round(output$p.value,5)` of 100000 to observe a larger absolute difference in means than what we observed in the data in a random sample that was generated under the null hypothesis. So it is extremely unlikely that our data would originate from a population where the average relative abundance of *Staphylococcus* in the microbiome upon placebo treatment would be equal that upon transplant treatment.  As such, we can reject the null
hypothesis extremely significant in favor of the alternative that there is an effect of the treatment in .

The mean relative abundance in the placebo group is `r round( unname(output$estimate[1]))`, 
while the mean relative abundance in the transplant group
is `r round( unname(output$estimate[2]))`. We further conclude that 95% confidence interval  [ `r round(output$conf.int[c(1,2)],2)` ]
holds the true difference in population means of 
relative abundances between subjects treated with placebo 
and subjects treated with a microbial transplant and we know that the construction of the confidence intervals are as such that their conclusion on the location of the average difference is correct with a probability of 95%.

* Conclusion *

We conclude that the mean 
relative abundance of *Staphylococcus* sp in the armpit microbiome of subjects that had a placebo treatment is extremely
...
## Get insight in how plots look for sample sizes similar to our dataset if all assumptions hold. 
...
normSim <- matrix(rnorm(sum(apRelSum$n)*nSamp,
              mean=c(rep(apRelSum$mean[1],apRelSum$n[1]),
                     ...
                     sd=sigma),nrow=sum(apRelSum$n)) %>% as.data.frame
