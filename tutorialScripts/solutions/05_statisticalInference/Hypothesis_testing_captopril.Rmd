...
1) Is the average systolic blood pressure (SBP)
before captopril treatment higher tahn 147 mmHg?

2) Is the average SBP before captopril treatment 
significantly different from the average SBP after
captopril treatment, given that the data is unpaired?

3) Is the average SBP before captopril treatment 
significanttly different from the average SBP after
captopril treatment, given that the data is paired?
...
question, we will need to perform some hypothesis testing.
...
elevated SBP levels, higher than 147 mmHg.Therefore, we want
to test whether or not the provided patients have an SBP level
that is greater than 147 mmHg. To test this, we will perform a
one-sample t-test, which will tell us if the average SBPb of
our patients is significantly greater thatn 147 mmHg on the 5%
significance level.
...
test if mean SBPb is significantly higher than 147 mmHg.

More specifically, we will test the null hypothesis;

H0: the mean SBPb is equal to 147 mmHg

versus the alternative hypothesis;

HA: the mean SBPb is greater than 147 mmHg

```{r}
SBPb <- captopril %>%
  pull(SBPb)

output1 <- t.test(SBPb,mu=147,alternative = "greater",conf.level = 0.95)
output1
```
...
We know that for this example, we have paired data; each patient's
SBP and DBP levels are measured twice, before and after captopril
treatment. But let's assume for now that the data is _unpaired_.
This corresponds to an experimental design where we would have 30
patients. We would measure 15 patient's BP levels only before
treatment and other patients BP levels we would measure only after
treatment.

### Check the assumptions

1. The observations are independent of each other (in both groups)
2. The data (SBPb) must be normally distributed (in both groups)
3. The variability in the within both groups is similar

The first assumption is met, as we may assume that there are no
specific patterns of correlation in our group of 30 randomly
select patients.

For the second assumption, we already know from the first question
that SBPb is normally distributed. We will now check this for SBPa.
...
variability of both groups. We can do this in two ways;
...
output2 <- t.test(SBPb,SBPa, var.equal = TRUE, conf.level = 0.95)
output2
```
...
equal to 0. We have found a p-value of `r round(unname(output2$p.value),5)`, 
which is smaller than 0.05. As such, we will reject the null
hypothesis at the 5% significanc level in favor of the
alternative.

The p-value was calculated from the t-test statistic value 
`r round(unname(output2$statistic),2)` on a t-distribution with 28 
(15+15-2) degrees of freedom.

The mean SBPb value is `r round(unname(output2$estimate[1]),2)` mmHg, 
while the mean SBPa value is `r round(unname(output2$estimate[2]),2)` mmHg. 
With 95% probability, the interval [`r round(output2$conf.int[c(1,2)],2)`]
holds the true difference in population means of SBP levels
between patients treated and not treated with captopril.

** Conclusion **

We may conclude that, on the 5% significance level, the mean 
SBP levels of patients before captopril treatment is 
significantly (p = `r round(unname(output2$p.value),5)`) higher than the mean 
SBP levels of  patients after captopril treatment. The SBP levels are on average 
`r round(unname(output2$estimate[1])-unname(output2$estimate[2]),2)` mmHg 
...
While for question two we assumed that the data was unpaired,
we know that we actually have paored data, for which we 
measured each patients BP values twice. Therefore, there
will be a strong correlation between the BP values before and
...
SBPa value will be comparatively high as well. We can also
...
This test reject the null hypothesis that the true 
correlation between the SBPb and SBPa values are equal
to 0 wtih extreme significance.

As such, we will now perform a paired t-test.

```{r}
output3 <- t.test(SBPb,SBPa, paired = TRUE)
output3
```
...
equal to 0. The p-value (p = `r round(unname(output3$p.value),5)`) has now become
...
We may conclude that, on the 5% significance level, the mean 
SBP levels of patients before captopril treatment is 
extremely significantly (p = `r round(unname(output3$p.value),5)`) higher than the mean 
SBP levels of patients after captopril treatment. The SBP levels 
are on average `r round(unname(output3$estimate),2)` mmHg
higher before treatment than after treatment (95% CI [`r round(output3$conf.int[c(1,2)],2)`]).
