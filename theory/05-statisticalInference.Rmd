...
femSamp10 %>% 
  ...
  group_by(sample)%>%
  summarize_at("log2Chol",list(mean=~mean(.,na.rm=TRUE),
              sd=~sd(.,na.rm=TRUE),
              n=function(x) x%>%is.na%>%`!`%>%sum)) %>%
  mutate(se=sd/sqrt(n)) ,
...
```{r}
meanMed10<- apply(femSamp10%>%log2, 2, function(x) c(mean(x),median(x))) %>% t %>% data.frame
...
```{r}
meanMed50<- apply(femSamp50%>%log2, 2, function(x) c(mean(x),median(x))) %>% t %>% data.frame
...
- If $\bar X$ is normally distributed the standard error has a good interpretation: the s.e. is the standard deviatie of the sample mean.
...
- We do this look at the distribution for experiments with 5 subjects. 

```{r}
femSamp5<-sapply(1:1000,function(j,x,size) sample(x,size),size=5,x=fem$DirectChol)

femSamp5[,1] %>% log2 %>% as.data.frame %>%
ggplot(aes(x=.))+
...
femSamp5 %>% log2 %>% colMeans %>% as.data.frame %>% ggplot(aes(x=.)) +
...
femSamp5 %>% log2 %>% colMeans %>% as.data.frame %>%  ggplot(aes(sample=.)) +
stat_qq() +
stat_qq_line() +
ggtitle("Means on 5 females")
```
