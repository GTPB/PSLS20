...
We will now create a subset of the data that we will use to sample from in the next sections.
  ...
  2. We only select the variables Gender and DirectChol from the dataset to avoid unnessary variables.
...
- Suppose that we have no access to cholestorol levels of the American population,
...
- Note, that in the last sample the log cholestorol levels are on average lower for females than for males; based on this sample we even would wrongly conclude that the cholestorol levels for females are on average larger than those of males.
...
- In 1916, the US experienced the first large epidemy of polio.
